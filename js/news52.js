...
const displayCatagoriesNews = async () => {
  const displayCatagories = await loadNewscategories();
  ...
  displayCatagories.data.news_category.forEach((data) => {
    ...
    div.innerHTML = `
          ...
          <button class="btn btn-link text-red-900" onclick="dynamicCategory(${data.category_id})">${data.category_name} </button>
          `;
